...
export default {
    ...
    async fetchHandlePlace(){
        const res = await this.$http.get(`places/${this.city}`)
        this.model = res.data
          //深拷贝
        this.placeList = JSON.parse(JSON.stringify(this.model[0].placeList))
        //为了获得一个扁平化数据的数组
    ...
    handleDownload () {
      ...
      var excelDatas = [
        {
           tHeader: ["编号",  "城市","处置场", "更新时间"], // sheet表一头部
          filterVal: ['_id', "city",'name',"updatedAt"], // 表一的数据字段
          tableDatas: this.placeList, // 表一的整体json数据
          sheetName: "sheet1"// 表一的sheet名字
        }
      ]
        //   引入的函数
      json2excel(excelDatas, "垃圾处置场所数据报表", true, "xlsx")
    },
    dateFormat(val) {
...
.info_content{
   ...
   section#table{
    ...
    border-top:45px solid rgb(255,233,179);
